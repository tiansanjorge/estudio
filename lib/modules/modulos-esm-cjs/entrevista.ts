...
b.getEstado(); // undefined — "a" y "b" son dos copias distintas del módulo`,
  ...
  },
  {
    ...
    respuestaEs:
      "require() en CJS devuelve el objeto module.exports del otro módulo tal como está en ese instante: en un ciclo, ese módulo quedó pausado antes de asignar, así que el objeto está incompleto. No es una copia: si B guarda el objeto y lee la propiedad más tarde, ve el valor final. Lo que falla es desestructurar en el require (const { valor } = require('./a')), que captura undefined, o que A reasigne module.exports con un objeto nuevo, porque B se queda con el viejo. Los named exports de ESM, en cambio, son bindings en vivo: si el valor se usa recién dentro de una función que se ejecuta DESPUÉS de que el ciclo de carga terminó (no en el nivel superior del módulo durante el ciclo), el binding ya refleja el valor final, porque no es una copia sino una referencia actualizada. El caso que sigue fallando en ambos sistemas es usar el valor directamente en el nivel superior mientras el ciclo todavía está en curso.",
    respuestaEn:
      "require() in CJS returns the other module's module.exports object as it is at that instant: in a cycle, that module is paused before assigning, so the object is incomplete. It's not a copy: if B keeps the object and reads the property later, it sees the final value. What breaks is destructuring at require time (const { valor } = require('./a')), which captures undefined, or A reassigning module.exports to a new object, because B keeps the old one. ESM's named exports, on the other hand, are live bindings: if the value is only used inside a function that runs AFTER the load cycle finished (not at the module's top level during the cycle), the binding already reflects the final value, because it's not a copy but an updated reference. The case that still breaks in both systems is using the value directly at the top level while the cycle is still in progress.",
    repregunta:
