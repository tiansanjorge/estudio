...
- ✅ MCP servers — qué son y para qué sirven (Nivel 1/2/3 + Entrevista)
- ✅ Riesgos y límites del desarrollo 100% asistido por IA (Nivel 1/2/3 + Entrevista)
- ✅ Cómo comunicar en entrevista tu metodología de trabajo con IA (Nivel 1/2/3 + Entrevista)
...
- ✅ **Revisión de quizzes** (2026-09-24, las 800 preguntas). Diagnóstico inicial: la correcta era la opción 2 en el 96% de las 800 preguntas, la más larga en el 90% y medía 2,6× los distractores.
  - ✅ Posición: `Quiz` mezcla las opciones con un orden determinístico por pregunta (hash del texto), así que la posición en pantalla ya es uniforme.
  - Distractores, por categoría. Criterios: cada distractor es un error plausible (el concepto aplicado en la condición equivocada, una confusión típica entre mecanismos parecidos, una verdad a medias); las 3 opciones tienen largo y estructura parecidos (a veces recortando la correcta); sin "siempre/nunca" solo en las incorrectas ni opciones absurdas. Meta por categoría: la correcta es la más larga en ~33–40% y la relación de largos queda cerca de 1×.
    - ✅ Las 19 categorías. Resultado global: la correcta es la más larga en el 45% (antes 90%) y la relación de largos es 1,02× (antes 2,6×). Por categoría va de 28% (Next.js) a 67% (Testing, casi todo opciones de una o dos palabras donde el largo no delata nada).
    - Se corrigió de paso un error de datos: en Hooks → useReducer, "¿Qué recibe y qué devuelve una función reducer?" marcaba como correcta una opción falsa.
    - Para preguntas nuevas: escribir la correcta concisa y los distractores con su largo como piso; cada distractor tiene que ser un error que alguien que entendió a medias elegiría.
