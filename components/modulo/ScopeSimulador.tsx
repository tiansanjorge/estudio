...
import type { EscenarioScope } from "@/lib/modules/scope/escenarios";

interface ScopeSimuladorProps {
  escenarios: EscenarioScope[];
...
export function ScopeSimulador({ escenarios, mostrarSelector = false }: ScopeSimuladorProps) {
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3">
        {paso.scopes.map((scope) => {
          const resaltado = scope.nombre === paso.scopeResaltado;
          return (
            <div
              key={scope.nombre}
            ...
            >
              <span
                className={`text-xs font-medium uppercase tracking-wide ${
                  resaltado ? "text-accent" : "text-muted-foreground"
                }`}
              >
                {scope.nombre}
              </span>
              {Object.keys(scope.variables).length === 0 ? (
                <span className="text-xs text-muted-foreground">sin variables propias</span>
              ) : (
